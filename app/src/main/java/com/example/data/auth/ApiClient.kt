...
object ApiClient {
    fun create(context: Context, baseUrl: String): ApiService {
        ...
        // Interceptor para agregar headers de trazabilidad
        ...
        val authenticator = object : Authenticator {
            ...
            override fun authenticate(route: Route?, response: Response): Request? {
                ...
                if (count >= 2) {
                    Log.w("ApiClient-Auth", "already attempted to authenticate, giving up")
                    return null
                }
                ...
                if (originalUrl.contains("/api/auth/login") || originalUrl.contains("/api/auth/refresh") || originalUrl.contains("/api/auth/oauth/")) {
                    Log.w("ApiClient-Auth", "not refreshing token for auth endpoints: $originalUrl")
                    return null
                }
                ...
                try {
                    ...
                    if (BuildConfig.DEBUG) {
                        Log.d("ApiClient-Auth", "refresh response body(length=${respBody.length})")
                    }
                    ...
                    if (newAccess.isNullOrEmpty()) {
                        Log.w("ApiClient-Auth", "refresh response had no access token -> clearing tokens")
                        TokenStorage.clear(context)
                        return null
                    }
                    ...
                    Log.e("ApiClient-Auth", "refresh token failed", ex)
                    TokenStorage.clear(context)
                    return null
                }
            }
